package com.jade.hust.activity;
...
import com.jade.hust.WifiAdapter;
import com.jade.hust.WifiScan;
import com.jade.hust.activity.AddPositionActivity.SaveListener;
...
public class MainActivity extends Activity {
	...
	private void getPosition() {
		...
		for(int i=0;i<5;i++){//取当前前5个wifi，根据SSID找到相应的位置，然后为每个位置打分。
			...
			map_result=WifiDB.getInstance(MainActivity.this).search(wifi.getBSSID());//查找出所有包含这个wifi信号的位置
			...
			if(map_score.get(key)==null){//如果当前位置没有打分，则把当前wifi为当前位置的打分存入
				...
				int val = (Integer) entry.getValue();	
				Toast.makeText(MainActivity.this, entry.getKey()+"..."+val,Toast.LENGTH_SHORT).show();
				...
				if(key==null)
				{
					Toast.makeText(MainActivity.this, "no data in map,please add postion first",Toast.LENGTH_SHORT).show();
