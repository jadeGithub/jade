...
import com.jade.hust.AddWifiAdapter;
	...
	public void onClick(DialogInterface dialog, int which) {
		...
		for(int i=0;i<list2.size();i++){
			...
			for(int j=1;j<6;j++){
			    ...
			    if(map_put.get(wifi1.getBSSID())==null){//判断这个wifi是否保存，如果没有保存这个wifi，则保存				
				 swf.setName(wifi1.getBSSID());
			     swf.setMax(wifi1.getLEVEL());			   
			     swf.setMin(wifi1.getLEVEL());
			     }
			   else {//如果有，则更新map_put中的数据				
				 swf=(StoreWifi) map_put.get(wifi1.getBSSID());//获取原来保存的swf数据
				 swf.setName(wifi1.getBSSID());
